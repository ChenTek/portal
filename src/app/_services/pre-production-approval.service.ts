...
@Injectable()
export class PreProductionApprovalService  {
    ...
    getResults(){
       // let header=new HttpHeaders({'Content-Type': 'application/json', 'Authorization': 'Bearer '+ JSON.parse(localStorage.getItem('currentUser')).map.token});
        return this.http.get<any>(this.baseURL +'/pre_production_approval') //, {headers: header}
            .map(result => { 
 ...
 getPage(filter = '', sortBy: string, sortOrder: string,
    ...
    // let header=new HttpHeaders({'Content-Type': 'application/json', 'Authorization': 'Bearer '+ JSON.parse(localStorage.getItem('currentUser')).map.token});
    return this.http.get<any>(this.baseURL +'/pre_production_approval',
      {
